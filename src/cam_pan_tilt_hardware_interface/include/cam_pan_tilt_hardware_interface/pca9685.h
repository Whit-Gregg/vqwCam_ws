...
namespace PCA9685
{
    ...
    {
      private:
        std::array<uint8_t, 2> temporary_2byte {0, 0};
        std::array<uint8_t, 8> pwm_tx_data {0, 0, 0, 0, 0, 0, 0, 0};
      ...
      public:
        PCA9685_Controller(const std::string &_i2c_dev_name, const uint8_t &_slave_address);
        void sleep();
        void wake_up();
        void start();
