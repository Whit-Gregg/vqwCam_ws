#include "cam_pan_tilt_hardware_interface/pca9685.h"

using namespace PCA9685;

PCA9685_Controller::PCA9685_Controller(const std::string &_i2c_dev_name, const uint8_t &_slave_address)
    : I2CController(_i2c_dev_name),
      on_lo_register(pwm_tx_data[0]),
      on_lo_value(pwm_tx_data[1]),
      on_hi_register(pwm_tx_data[2]),
      on_hi_value(pwm_tx_data[3]),
      off_lo_register(pwm_tx_data[4]),
      off_lo_value(pwm_tx_data[5]),
      off_hi_register(pwm_tx_data[6]),
      off_hi_value(pwm_tx_data[7])
{
...
{
    for (size_t i = 0; i < 8; i += 2) { write_bytes(&(pwm_tx_data[i]), 2); }
}
